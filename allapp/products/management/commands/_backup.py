...
def _as_gtin(value: Any) -> Optional[str]:
    ...
    # 兼容 Excel 把条码读成 1234567890123.0
    if re.fullmatch(r"\d+\.0", s):
        s = s[:-2]

    # Product.clean 里也会校验，这里先提前给出更清晰错误
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--update",
            action="store_true",
            help="若 owner+code 已存在，是否更新 name/base_uom/extra/expiry_control/shelf_life_days 等非标识字段",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
            help="只校验，不提交数据库。注意：会在事务里模拟创建，再整体回滚。",
        )
            ...
            for row_no, values in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
                ...
                try:
                    with transaction.atomic():
                        ...
                        # 效期规则：
                        # 1. 如果 expiry_control 明确填了，是/否以它为准；
                        # 2. 如果没填，但 shelf_life_days > 0，则自动开启效期；
                        # 3. 否则关闭效期。
