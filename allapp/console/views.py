...
from allapp.tasking.models import TaskScanLog, WmsTask
...
def _scoped_inventory_transactions(request):
...
def _filtered_qs(request):
    qs = _scoped_inventory_transactions(request)
    q = (request.GET.get("q") or "").strip()
    if q:
        qs = qs.filter(
            Q(src_no__icontains=q)
            | Q(posting_batch__icontains=q)
            | Q(owner__name__icontains=q)
            | product_search_q(q, product_field="product_id")
        )
    return qs, q
...
def tx_toggle_flag(request, pk):
    ...
    obj = get_object_or_404(_scoped_inventory_transactions(request), pk=pk)
    # 假设有个布尔字段 verified
...
def pick_task_list(request):
    tasks = (
        WmsTask.objects.filter(task_type="PICK", status__in=["READY", "IN_PROGRESS"])
...
class PickTaskListView(HtmxMixin, ListView):
    ...
    def get_queryset(self):
        qs = (
            WmsTask.objects.filter(task_type="PICK")
            .select_related("warehouse", "owner")
            .order_by("priority", "id")
        )
...
class PickScanView(View):
    ...
    def post(self, request, pk):
        ...
        TaskScanLog.objects.get_or_create(
            ...
            barcode=barcode,
            fp=fp,
            defaults={"qty": qty, "created_by": request.user},
        )
        # 回前端：1) 触发刷新行表；2) OOB 弹出 Toast
        from django.template.loader import render_to_string

        oob_toast = render_to_string(
            "console/_toast_success.html", {"msg": f"已接收：{barcode} × {qty}"}
        )
        html_lines = render_to_string(
            "console/pick/_lines.html",
            {
                "task": task,
                "lines": task.wmstaskline_set.select_related("product", "location"),
            },
            request=request,
        )
...
class PickPostView(View):
    ...
    def post(self, request, pk):
        task_services._run_posting_handler(pk, by_user=request.user, note="控制台过账")
