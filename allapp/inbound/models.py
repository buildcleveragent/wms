...
def _q_qty_q(x):
    """将任意数值量化到 3 位（数量口径）"""
    return (Decimal(x) if isinstance(x, Decimal) else Decimal(str(x))).quantize(
        QTY_Q, rounding=ROUND_HALF_UP
    )
...
def _infer_base_uom_code(owner_id, product_code):
    ...
    try:
        ...
        product = (
            Product.objects.filter(owner_id=owner_id, code=product_code)
        ...
        if not product or not product.get("base_uom__code"):
            logger.warning(
                "Product with owner_id=%s and code=%s does not have a base_uom.",
                owner_id,
                product_code,
            )
            return None
        ...
        # 捕获可能发生的异常并记录错误
        logger.error(
            "Error fetching base_uom code for owner_id=%s, product_code=%s: %s",
            owner_id,
            product_code,
            e,
        )
...
class InboundOrder(BaseModel):
    ...
    inbound_type = models.CharField(
        "入库类型", max_length=15, choices=INBOUND_TYPE_CHOICES, default="PURCHASE"
    )
    delivery_method = models.CharField(
        "交货方式", max_length=15, choices=DELIVERY_METHOD_CHOICES, blank=True, null=True
    )
    eta = models.DateTimeField("预计到货时间", blank=True, null=True)

    submit_status = models.CharField(
        "提交状态", max_length=15, choices=SUBMIT_CHOICES, default="DRAFT", db_index=True
    )
    approval_status = models.CharField(
        "审核状态", max_length=15, choices=APPROVAL_CHOICES, default="NOT_READY", db_index=True
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=["owner", "biz_date", "submit_status"], name="ii_order_owner_date_status"
            ),
        ...
        constraints = [
            models.CheckConstraint(
                ...
                condition=Q(submit_status__in=["DRAFT", "SUBMITTED"]),
            ),
            models.CheckConstraint(
                condition=models.Q(
                    approval_status__in=[
            ...
            models.CheckConstraint(
                # 已关闭订单，不能取消
                condition=~(Q(is_closed=True) & Q(approval_status__in=["CANCELLED"])),
    ...
    def wh_confirm(self, user):
        ...
        self.save(
            update_fields=["approval_status", "approved_by_warehouse", "approved_at_warehouse"]
        )
...
class InboundOrderLine(BaseModel):
    order = models.ForeignKey(
        "InboundOrder", verbose_name="订单", on_delete=models.PROTECT, related_name="lines"
    )
    ...
    aux_price = models.DecimalField(
        "包装价格", max_digits=14, decimal_places=4, blank=True, null=True
    )
    aux_qty = models.DecimalField(
        "包装数量", max_digits=18, decimal_places=3, blank=True, null=True
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                ...
                condition=Q(aux_uom__isnull=True) | Q(ratio__gt=0),
            ),
            # 同单据内行号唯一
            models.UniqueConstraint(fields=["order", "line_no"], name="ux_inb_order_line_no"),
            # 至少一个数量 > 0
            models.CheckConstraint(
                condition=(Q(base_qty__gt=0) | Q(aux_qty__gt=0)),
                name="chk_inb_qty_one_positive",
            ),
            # 两个数量都不得为负
            models.CheckConstraint(
                condition=Q(base_qty__gte=0) & (Q(aux_qty__gte=0) | Q(aux_qty__isnull=True)),
                name="chk_inb_qty_non_negative",
            ),
            # 价格非负
            models.CheckConstraint(
                condition=Q(base_price__gte=0) & (Q(aux_price__gte=0) | Q(aux_price__isnull=True)),
    ...
    def clean(self):
        ...
        if self.aux_uom_id and self.aux_qty and self.base_qty:
            ...
            if (expect_base - self.base_qty).copy_abs() > Decimal("0.001"):
                errors["base_qty"] = (
                    f"基本数量与包装数量不一致，应为 {expect_base}（当前 {self.base_qty}）。"
                )
    ...
    def _ensure_line_no(self):
        ...
        with transaction.atomic():
            ...
            if not order.next_line_no:
                last = (
                    type(self).objects.filter(order_id=order.id).aggregate(m=Max("line_no"))["m"]
                    or 0
                )
    ...
    def save(self, *args, **kwargs):
        ...
        # self.line_amount = (
        #     Decimal(self.base_qty or 0) * unit_price_base
...
class InboundReceipt(BaseModel):
    ...
    inbound_type = models.CharField(
        "入库类型", max_length=16, choices=INBOUND_TYPE_CHOICES, default="PURCHASE"
    )
    delivery_method = models.CharField(
        "交货方式", max_length=16, choices=DELIVERY_METHOD_CHOICES, blank=True, null=True
    )
    submit_status = models.CharField(
        "提交状态", max_length=16, choices=SUBMIT_CHOICES, default="DRAFT", db_index=True
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                ...
                condition=Q(approved_by__isnull=True) | Q(submit_status="SUBMITTED"),
            ),
            # 已提交必须有制单人（按你的流程决定是否需要）
            models.CheckConstraint(
                name="chk_rcpt_submit_need_maker",
                condition=~Q(submit_status="SUBMITTED", made_by__isnull=True),
            ),
        ]
        indexes = [
            models.Index(
                fields=["owner", "submit_status", "biz_date"], name="ix_rcpt_own_stat_date"
            ),
...
class InboundReceiptLine(BaseModel):
    ...
    order_qty = models.DecimalField(
        "订购数量", max_digits=14, decimal_places=3, blank=True, null=True
    )
    base_qty = models.DecimalField(
        "本次入库数(基本单位)", max_digits=14, decimal_places=3, default=0
    )
    ...
    pack_size = models.DecimalField(
        "换算(基本单位/包,快照)", max_digits=18, decimal_places=4, blank=True, null=True
    )
    pack_qty = models.DecimalField(
        "本次包数(快照)", max_digits=14, decimal_places=3, blank=True, null=True
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(condition=Q(line_no__gte=1), name="ck_rcpt_lineno_pos"),
            # 数量/单价非负
            models.CheckConstraint(
                condition=Q(base_qty__gte=0),
            ...
            models.CheckConstraint(
                condition=Q(exp_date__isnull=True)
                | Q(mfg_date__isnull=True)
                | Q(exp_date__gte=F("mfg_date")),
                name="ck_rcpt_exp_ge_mfg",
            ),
            # 包装一致性：有包数就必须有正的换算
            models.CheckConstraint(
                condition=Q(pack_qty__isnull=True)
                | (Q(pack_size__isnull=False) & Q(pack_size__gt=0)),
    ...
    def _get_product(self):
        ...
        # 预取 base_uom，避免随后访问 p.base_uom.code 时产生 N+1 查询。
        return (
            Product.objects.filter(owner_id=owner_id, code=self.product_code)
            ...
            if base_val == 0:
                ...
                if (base_val - calc_base).copy_abs() > Decimal("0.0005"):
                    raise ValidationError(
                        {
                            "base_qty": f"基本数量与包装换算不一致，应为 {calc_base}（当前 {base_val}）。"
                        }
                    )
...
class InboundOrderReturnInfo(BaseModel):
    ...
    source_channel = models.CharField(
        "来源渠道", max_length=20, choices=SOURCE_CHANNELS, blank=True, null=True
    )
    orig_outbound_order_no = models.CharField("原出库单号", max_length=40, blank=True, null=True)

    reason_code = models.CharField(
        "退货原因", max_length=20, blank=True, null=True
    )  # 或 FK 到“原因字典”
    photos_required = models.BooleanField("需要照片", default=False)

    refund_amount = models.DecimalField(
        "退款金额", max_digits=14, decimal_places=2, blank=True, null=True
    )
    refund_status = models.CharField(
        "退款状态", max_length=20, choices=REFUND_STATUSES, blank=True, null=True
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                ...
                condition=Q(refund_amount__isnull=True) | Q(refund_amount__gte=0),
            ),
        ]
        indexes = [
...
class ReturnInspection(BaseModel):
    ...
    owner = models.ForeignKey(
        "baseinfo.Owner", on_delete=models.PROTECT, related_name="return_inspections"
    )
    warehouse = models.ForeignKey(
        "locations.Warehouse",
        on_delete=models.PROTECT,
    ...
    condition = models.CharField(
        "品相", max_length=10, choices=CONDITION_CHOICES, blank=True, null=True
    )
    ...
    status = models.CharField(
        "状态", max_length=10, choices=STATUS_CHOICES, default="OPEN", db_index=True
    )
    ...
    class Meta:
        ...
        constraints = [
            # 数量必须为正
            models.CheckConstraint(condition=Q(qty__gt=0), name="chk_retinsp_qty_pos"),
            # 若有序列号，数量必须为 1（空串视为无序列号，在 save() 里归一到 None）
            models.CheckConstraint(
                condition=Q(serial_no__isnull=True) | Q(qty=1),
        ...
        indexes = [
            models.Index(
                fields=["warehouse", "status", "disposition"], name="idx_retinsp_wh_status_disp"
            ),
    ...
    def clean(self):
        ...
        if self.status in {"APPROVED", "POSTED"} and (
            not self.inspected_by_id or not self.inspected_at
        ):
...
class Lot(BaseModel):
    owner = models.ForeignKey(
        "baseinfo.Owner", verbose_name="货主", on_delete=models.PROTECT, related_name="lots"
    )

    # 商品：允许为空，但要求提供 product_code（快照）
    product = models.ForeignKey(
        "products.Product", on_delete=models.PROTECT, null=True, blank=True, related_name="lots"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=["owner", "product_code", "exp_date"], name="idx_lot_owner_prod_exp"
            ),
            models.Index(
                fields=["owner", "supplier", "product_code"], name="idx_lot_owner_supp_prod"
            ),
        ]
...
class LotWarehouse(BaseModel):
    owner = models.ForeignKey(
        "baseinfo.Owner", on_delete=models.PROTECT, related_name="lot_warehouses"
    )
    lot = models.ForeignKey("Lot", on_delete=models.PROTECT, related_name="warehouses")
    warehouse = models.ForeignKey(
        "locations.Warehouse", on_delete=models.PROTECT, related_name="lot_warehouses"
    )
    ...
    qa_status = models.CharField(
        "质检状态", max_length=20, blank=True, null=True, choices=QA_STATUS
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                ...
                | (Q(blocked_until__isnull=False) & Q(blocked_reason__isnull=False)),
            ),
        ]
        indexes = [
            ...
            models.Index(
                fields=["owner", "warehouse", "qa_status"], name="idx_lotwh_owner_wh_qastat"
            ),
        ]
    ...
    def clean(self):
        ...
        # if self.qa_status and self.qa_status not in {
        #     "RELEASED", "HOLD", "QUARANTINE", "RETURN", "SCRAP_PENDING"
        # }:
