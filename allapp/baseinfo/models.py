...
from allapp.core.models import AddressMixin, BaseModel

phone_validator = RegexValidator(
    regex=r"^[0-9+\-\s()]{5,20}$", message="电话仅允许数字、+、-、空格、()，长度5-20。"
)
...
class Customer(BaseModel, AddressMixin):
    ...
    delivery_seq = models.PositiveIntegerField(
        "配送序号", blank=True, null=True, validators=[MinValueValidator(1)]
    )
    level = models.PositiveSmallIntegerField(
        "客户等级", blank=True, null=True, validators=[MinValueValidator(1)]
    )
    ...
    promised_days = models.PositiveSmallIntegerField(
        "应达天数", blank=True, null=True, validators=[MinValueValidator(0)]
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                condition=Q(delivery_distance_km__gte=0) | Q(delivery_distance_km__isnull=True),
                name="chk_cust_delivery_dist_pos",
            ),
            models.CheckConstraint(
                condition=Q(promised_days__gte=0) | Q(promised_days__isnull=True),
        ...
        indexes = [
            ...
            # 7) 配送线路内按序号排序/查询
            models.Index(
                fields=["owner", "delivery_route", "delivery_seq"], name="idx_customer_route_seq"
            ),
...
class Employee(BaseModel):
    ...
    phone = models.CharField(
        "联系电话", max_length=20, blank=True, null=True, validators=[phone_validator]
    )
    mobile = models.CharField(
        "移动电话", max_length=20, blank=True, null=True, validators=[phone_validator]
    )
...
class CarrierCompany(BaseModel, AddressMixin):
    name = models.CharField("承运公司名称", max_length=30, unique=True, db_index=True)
    ...
    class Meta:
        ...
        # constraints = [
        #     models.UniqueConstraint(
        #         fields=["mobile"], name="uq_carrier_mobile"
        #     ),  # Ensure mobile is unique
        #     models.CheckConstraint(
        #         condition=models.Q(mobile__regex=r'^\+?[1-9]\d{1,14}$'),
...
class Driver(BaseModel, AddressMixin):
    ...
    gender = models.CharField(
        "性别", max_length=1, choices=[("M", "男"), ("F", "女")], blank=True, null=True
    )
    mobile = models.CharField(
        "手机号码", max_length=20, blank=True, null=True, validators=[phone_validator]
    )
    phone = models.CharField("电话号码", max_length=20, blank=True, null=True)
...
class Vehicle(BaseModel):
    ...
    payload_kg = models.DecimalField(
        "可载重量(kg)", max_digits=10, decimal_places=2, blank=True, null=True
    )
    surcharge_cert_no = models.CharField("附加费证号", max_length=40, blank=True, null=True)
    length_m = models.DecimalField(
        "车长(米)", max_digits=6, decimal_places=2, blank=True, null=True
    )
    volume_m3 = models.DecimalField(
        "可载体积(m³)", max_digits=10, decimal_places=3, blank=True, null=True
    )
    maintenance_km = models.PositiveIntegerField("保养公里数", blank=True, null=True)
    status = models.CharField(
        "车辆状态",
        max_length=20,
        choices=[("IDLE", "空闲"), ("ON_DUTY", "在岗"), ("MAINT", "保养"), ("DISABLED", "停用")],
        default="IDLE",
    )
...
class DictItem(BaseModel):
    category = models.ForeignKey(
        "DictCategory", verbose_name="所属分类", on_delete=models.PROTECT, related_name="items"
    )
