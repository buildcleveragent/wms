...
def _decimal_ranges_overlap(start_a, end_a, start_b, end_b) -> bool:
    return Decimal(start_a) < _decimal_range_end(end_b) and Decimal(
        start_b
    ) < _decimal_range_end(end_a)
...
class BillingRule(BillingValidationMixin, models.Model):
    ...
    effective_from = models.DateField(
        verbose_name=_("生效开始日"), null=True, blank=True
    )
    effective_to = models.DateField(verbose_name=_("生效截止日"), null=True, blank=True)
    note = models.CharField(
        verbose_name=_("备注"), max_length=200, blank=True, default=""
    )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                ...
                check=models.Q(tax_rate__gte=0, tax_rate__lte=1),
            ),
            models.CheckConstraint(
                name="chk_rule_min_charge_nonneg", check=models.Q(min_charge__gte=0)
            ),
            models.CheckConstraint(
            ...
            models.CheckConstraint(
                ...
                check=models.Q(bundle_price__isnull=True)
                | models.Q(bundle_price__gte=0),
            ),
        ]
    ...
    def clean(self):
        ...
        if self.bundle_scope in {None, "", BundleScope.NONE}:
            ...
            if self.bundle_price is not None:
                errors["bundle_price"] = (
                    "bundle_scope 为 NONE 时，bundle_price 必须为空。"
                )
        else:
        ...
        if self.unit_price is None and not has_tiers and self.ladder_mode is None:
            ...
            errors["unit_price"] = (
                "启用阶梯模式但尚无阶梯配置时，必须填写 unit_price 作为兜底。"
            )

        if (
            self.calc_method == CalcMethod.PERCENT_OF_ORDER_AMOUNT
            and self.unit_price is not None
        ):
            if not (Decimal("0") <= Decimal(self.unit_price) <= Decimal("1")):
                errors["unit_price"] = (
                    "按订单金额比例计费时，unit_price 必须在 0~1 之间。"
                )
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                ...
                check=models.Q(threshold_to__isnull=True)
            ...
            models.CheckConstraint(
                name="chk_tier_from_nonneg", check=models.Q(threshold_from__gte=0)
            ),
            models.CheckConstraint(
            ...
            models.CheckConstraint(
                ...
                check=models.Q(percent_rate__isnull=True)
    ...
    def clean(self):
        ...
        if self.rule_id:
            if self.rule.calc_method == CalcMethod.PERCENT_OF_ORDER_AMOUNT:
                if self.unit_price is not None:
                    errors["unit_price"] = (
                        "按订单金额比例的阶梯规则只允许填写 percent_rate。"
                    )
...
class BillingServiceContract(BillingValidationMixin, models.Model):
    ...
    owner = models.ForeignKey(
        "baseinfo.Owner", verbose_name=_("货主"), on_delete=models.PROTECT
    )
    warehouse = models.ForeignKey(
    ...
    source_type = models.CharField(
        verbose_name=_("来源类型"), max_length=40, default="TASK"
    )
...
class BillingPeriod(BillingValidationMixin, models.Model):
    ...
    invoiced_at = models.DateTimeField(
        verbose_name=_("开票时间"), null=True, blank=True
    )
...
class BillingEvent(BillingValidationMixin, models.Model):
    ...
    quantity_uom = models.CharField(
        verbose_name=_("数量单位"), max_length=20, default="BASE"
    )
    ...
    pricing_detail = models.JSONField(
        verbose_name=_("计价明细"), blank=True, default=dict
    )
    priced_at = models.DateTimeField(
        verbose_name=_("计价完成时间"), null=True, blank=True
    )
    ...
    def clean(self):
        ...
        if self.task_id:
            if (
                self.task.owner_id != self.owner_id
                or self.task.warehouse_id != self.warehouse_id
            ):
        ...
        if self.task_line_id:
            ...
            if (
                ...
                errors["task_line"] = (
                    "task_line 的 owner/warehouse 必须与计费事件一致。"
                )
        ...
        if self.posting_journal_id:
            ...
            if not expected_task_id:
                ...
                if (
                    ...
                    errors["posting_journal"] = (
                        "posting_journal 必须与当前事件关联的 task 一致。"
                    )
...
class BillingAccrual(BillingValidationMixin, models.Model):
    ...
    currency = models.CharField(verbose_name=_("币种"), max_length=8, default="CNY")
    quantity = models.DecimalField(
        verbose_name=_("计费数量"), max_digits=18, decimal_places=4
    )
    unit_price = models.DecimalField(
        verbose_name=_("有效单价/费率"), max_digits=18, decimal_places=4
    )
    amount = models.DecimalField(
        verbose_name=_("金额(不含税)"), max_digits=18, decimal_places=2
    )
    ...
    acc_fingerprint = models.CharField(
        verbose_name=_("应计指纹"), max_length=160, unique=True
    )
    created_at = models.DateTimeField(verbose_name=_("创建时间"), auto_now_add=True)
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                name="chk_qty_nonneg", check=models.Q(quantity__gte=0)
            ),
            models.CheckConstraint(
            ...
            models.CheckConstraint(
                ...
                check=~models.Q(status=AccrualStatus.LOCKED)
                | models.Q(period__isnull=False),
            ),
        ]
    ...
    def clean(self):
        ...
        if self.rule_id:
            ...
            if (
                self.rule.warehouse_id is not None
                and self.rule.warehouse_id != self.warehouse_id
            ):
            ...
            if (
                self.rule.currency
                and self.currency
                and self.rule.currency != self.currency
            ):
        ...
        if self.period_id:
            ...
            if not (
                self.period.start_date <= self.service_date <= self.period.end_date
            ):
                errors["service_date"] = "service_date 必须落在 period 区间内。"
            if (
                self.period.currency
                and self.currency
                and self.period.currency != self.currency
            ):
                currency_errors.append("currency 必须与 period.currency 一致。")

        if self.event_id:
            if (
                self.event.owner_id != self.owner_id
                or self.event.warehouse_id != self.warehouse_id
            ):
                errors["event"] = "event 的 owner/warehouse 必须与 accrual 一致。"
            if (
                self.event.charge_type != self.charge_type
                and "charge_type" not in errors
            ):
...
class BillingMetricDaily(BillingValidationMixin, models.Model):
    ...
    value = models.DecimalField(
        verbose_name=_("指标值"), max_digits=18, decimal_places=4
    )
    source = models.CharField(
        verbose_name=_("来源"), max_length=40, blank=True, default=""
    )
    source_quality = models.CharField(
        verbose_name=_("来源质量"),
        max_length=20,
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["owner", "warehouse", "service_date", "metric_type"])
        ]
        constraints = [
            models.UniqueConstraint(
            ...
            models.CheckConstraint(
                name="chk_metric_value_nonneg", check=models.Q(value__gte=0)
            ),
        ]
...
class BillingJobRun(BillingValidationMixin, models.Model):
    ...
    job_name = models.CharField(
        verbose_name=_("作业名"), max_length=40, choices=JobName.choices
    )
    service_date = models.DateField(verbose_name=_("服务日期"))
    status = models.CharField(
        verbose_name=_("执行状态"),
    ...
    finished_at = models.DateTimeField(
        verbose_name=_("结束时间"), null=True, blank=True
    )
    message = models.CharField(
        verbose_name=_("执行消息"), max_length=200, blank=True, default=""
    )
...
class Bill(BillingValidationMixin, models.Model):
    ...
    invoice_no = models.CharField(
        verbose_name=_("发票/结算单号"), max_length=40, unique=True
    )
    issue_date = models.DateField(
        verbose_name=_("开票日期"), default=bill_issue_date_default
    )
    ...
    def clean(self):
        ...
        if self.period_id:
            ...
            if (
                self.currency
                and self.period.currency
                and self.currency != self.period.currency
            ):
    ...
    def paid_amount(self):
        ...
        for allocation in self.payment_allocations.all():
            if allocation.receipt.status not in {
                ...
                continue
            posted += (
                -allocation.amount if allocation.is_reversal else allocation.amount
            )
...
class BillLine(BillingValidationMixin, models.Model):
    ...
    description = models.CharField(
        verbose_name=_("行说明"), max_length=200, blank=True, default=""
    )
    ...
    def clean(self):
        ...
        if self.accrual_id:
            ...
            if self.service_date != self.accrual.service_date:
                errors["service_date"] = (
                    "service_date 必须与 accrual.service_date 一致。"
                )
...
class PaymentReceipt(BillingValidationMixin, models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                condition=models.Q(amount__gt=0), name="chk_receipt_amount_pos"
            )
        ]
        indexes = [
            models.Index(
...
class PaymentAllocation(BillingValidationMixin, models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                condition=models.Q(amount__gt=0), name="chk_alloc_amount_pos"
            ),
            models.UniqueConstraint(
        ...
        indexes = [
            models.Index(fields=["bill", "allocated_at"], name="ix_alloc_bill_time")
        ]
...
class ReceivableCollectionCase(BillingValidationMixin, models.Model):
    ...
    bill = models.OneToOneField(
        Bill, on_delete=models.PROTECT, related_name="collection_case"
    )
