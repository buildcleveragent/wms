...
export const api = {
  ...
  // 审核列表（支持 search / approval_status / submit_status / page）
	...
	// PDA 拣货过账
	postPickTask: (id) =>
		request({ url: `/api/pda/pick-tasks/${id}/post/`, method: 'POST' }),
		
		  // PDA 拣货创建复核任务（拣货员点击“完成”时调用）
    createPickReviewTask: (id) =>
        request({ url: `/api/pda/pick-tasks/${id}/create-review-task/`, method: 'POST' }),
		
	  // ✅ PDA 待复核拣货任务列表（当前登录人可复核的）
	reviewPickTasks: (params = {}) =>
	    request({
	      ...
	      data: {
	        ...
	        for_review: 1,
	        ...params, // 以后要加分页/搜索可以从这里扩展
	      },
	    }),

	// utils/request.ts 里的 api
