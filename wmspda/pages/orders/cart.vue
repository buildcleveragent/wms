   ...
   <view class="content" v-if="cart.items.length">
	  ...
	  <view v-for="(it, i) in cart.items" :key="it.product_id ?? i"  :class="['row item', { 'odd': i % 2 === 0 }]" >
        ...
        <view class="col-info">
		  ...
		  <view class="product-spec" v-if="it.spec">
		    规格：{{ it.spec }}
		  </view>
          <view class="meta-container">
			...
			<text v-if="it.lot_no || it.batch_number" class="meta">批次: {{ it.lot_no || it.batch_number }}</text>
			<text v-if="it.mfg_date || it.production_date" class="meta">生产日期: {{ it.mfg_date || it.production_date }}</text>
			<text v-if="it.exp_date || it.expiry_date" class="meta">有效截止: {{ it.exp_date || it.expiry_date }}</text>
		  </view>
...
//   // 后端打印页 URL，如 https://wms.example.com/console/receive_task/58/print/
...
function compactReceiveItem(it) {
  ...
  const lotNo = (it.lot_no || it.batch_number || '').trim().toUpperCase()
  const mfgDate = (it.mfg_date || it.production_date || '').trim()
  const expDate = (it.exp_date || it.expiry_date || '').trim()

  if (lotNo) {
    item.lot_no = lotNo
    item.batch_number = lotNo
  }
  if (mfgDate) {
    item.mfg_date = mfgDate
    item.production_date = mfgDate
  }
  if (expDate) {
    item.exp_date = expDate
    item.expiry_date = expDate
  }
  return item
}
...
async function submitOrder(){
  ...
  try{
   ...
   //    }))
	  ...
	  items: cart.items.map(compactReceiveItem)
	  
    }
	
	console.log("aaaaaaaaaa111222 payload=",payload)
	console.log("receive_without_order payload json=", JSON.stringify(payload))
