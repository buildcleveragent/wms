...
<!--          <view class="cell col-num">{{ item.onhand_qty }}</view>
      ...
      <view class="pager">
        ...
        <view class="pager-size">
          ...
          >
            <view class="page-size-box">{{ pageSize }} 条</view>
          </picker>
        </view>

        <view class="pager-actions">
          <button class="pager-btn" size="mini" :disabled="page <= 1" @click="prevPage">
            上一页
          </button>
          <button class="pager-btn" size="mini" :disabled="page >= totalPages" @click="nextPage">
            下一页
          </button>
        </view>
      </view>
...
function onPageSizeChange(e) {
  ...
  pageSize.value = pageSizeOptions[idx] || 10
  page.value = 1
  load()
}
