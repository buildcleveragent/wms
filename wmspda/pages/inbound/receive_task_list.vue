<template>
  <view class="page">
    <view class="intro">
      <text class="intro-title">有订单收货</text>
      <text class="intro-copy">领取任务后，逐行登记良品、破损和拒收数量。</text>
    </view>
    ...
    <view v-if="loading && !rows.length" class="state">正在加载收货任务…</view>
    <view v-else-if="!rows.length" class="state">暂无可见的有订单收货任务</view>
    ...
    <view v-else class="task-list">
      <view v-for="task in rows" :key="task.id" class="task-card" @click="openTask(task)">
        ...
        <view class="line-summary">
          {{ task.lines?.length || 0 }} 行 · 已收 {{ receivedQty(task) }} / 计划 {{ plannedQty(task) }}
        </view>
...
function openTask(task) {
  uni.navigateTo({ url: `/pages/inbound/receive_task_detail?task_id=${task.id}` })
}
