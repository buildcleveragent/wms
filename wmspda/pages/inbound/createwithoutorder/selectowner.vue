...
<!-- 		  <view>
		...
		<view class="mt-6">
			<button class="btn" :disabled="!cart.owner" >下一步：记录供应商</button>
		</view>
	</view>
</template>
...
function choose(c){
  if (!c || !c.id) return
  cart.setOwner({ id: c.id, name: c.name })
  alive = false; reqSeq++
  // 用 redirectTo 可减少历史栈干扰
  // console.log(" goto /pages/inbound/selectsupplier")
  // uni.redirectTo({ url: '/pages/inbound/selectsupplier' })
