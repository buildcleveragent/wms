      ...
      <view v-for="(p, i) in rows" :key="p?.id ?? i" :class="['row item', { 'odd': i % 2 === 0 }]">
        ...
        <view class="col-info">
          <view class="name-price-row">
		    ...
		    <view class="product-spec" v-if="p?.spec">
		      规格：{{ p.spec }}
		    </view>
...
function cleanLot(v: any): string {
  return cleanText(v).toUpperCase()
}

function cartLotNo(it: any): string {
  return cleanLot(it?.lot_no || it?.batch_number)
}

function cartMfgDate(it: any): string {
  return cleanText(it?.mfg_date || it?.production_date)
}

function cartExpDate(it: any): string {
  return cleanText(it?.exp_date || it?.expiry_date)
}

function fillCartItemDates(it: any, batch: string, productionDate: string, expiryDate: string) {
  if (!it) return
  it.lot_no = batch
  it.batch_number = batch
  it.mfg_date = productionDate
  it.production_date = productionDate
  it.exp_date = expiryDate
...
function add(p: any) {
  ...
  const batch = cleanLot(batchMap.value[p.id]); // Get the batch from batchMap
  const productionDate = cleanText(productionDateMap.value[p.id]); // Get the production date
  const expiryDate = cleanText(expiryDateMap.value[p.id]); // Get the expiry date
  console.log('receive item dates=', { batch, productionDate, expiryDate })
  ...
  // Same product with different lot/date must stay as separate cart lines.
  const idx = cart.items.findIndex((x: any) =>
    x.id === p.id &&
    cartLotNo(x) === batch &&
  ...
  if (idx > -1) {
    ...
    fillCartItemDates(cart.items[idx], batch, productionDate, expiryDate)
    ...
    fillCartItemDates(cart.items[cart.items.length - 1], batch, productionDate, expiryDate)
