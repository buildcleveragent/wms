    ...
    <view class="gs1-sheet" data-testid="gs1-quick-create-modal">
      ...
      <scroll-view scroll-y class="gs1-body">
        ...
        <view class="form-section">
          ...
          <view class="option-grid" data-testid="gs1-category-grid">
            <view
              v-for="item in filteredCategories"
              :key="item.id"
              :class="['option-chip', form.category_id === item.id ? 'selected' : '']"
          ...
          <view class="option-grid">
            <view
              ...
              :key="item.id"
              :class="['option-chip', form.base_uom_id === item.id ? 'selected' : '']"
        ...
        <view class="form-section">
          ...
          <view class="tracking-block">
            <text class="tracking-title">批次管理</text>
            <text class="tracking-help">填写批次号后自动启用；留空则不启用</text>
            <text class="field-label">本次批号（选填）</text>
            <input v-model="form.lot_no" class="field-input" placeholder="扫描或输入包装批号" data-testid="gs1-lot-no" />
          </view>

          <view class="tracking-block" data-testid="gs1-expiry-fields">
            <text class="tracking-title">效期管理</text>
            <text class="tracking-help">填写任一效期信息后自动启用；全部留空则不启用</text>
            <text class="field-label">效期基准（启用后必选）</text>
            ...
            <template v-if="form.expiry_basis === 'MFG'">
              <text class="field-label">保质期天数（启用后必填）</text>
              <input v-model="form.shelf_life_days" class="field-input" type="number" placeholder="例如 365" data-testid="gs1-shelf-life" />
            </template>
            <template v-else>
              <text class="field-label">入库有效天数（启用后必填）</text>
              <input v-model="form.inbound_valid_days" class="field-input" type="number" placeholder="例如 30" data-testid="gs1-inbound-valid-days" />
            </template>
            <text class="field-label">效期预警天数</text>
            <input v-model="form.expiry_warning_days" class="field-input" type="number" placeholder="选填，必须小于有效天数" data-testid="gs1-expiry-warning" />

            <view class="date-row">
              <view class="date-col">
                <text class="field-label">生产日期{{ form.expiry_basis === 'MFG' ? '（启用后必填）' : '' }}</text>
                <picker mode="date" :value="form.mfg_date" data-testid="gs1-mfg-date" @change="form.mfg_date = $event.detail.value">
                  <view class="date-picker">{{ form.mfg_date || '选择日期' }}</view>
                </picker>
              </view>
              <view class="date-col">
                <text class="field-label">到期日期{{ form.expiry_basis === 'MFG' ? '（启用后必填）' : '' }}</text>
                <picker mode="date" :value="form.exp_date" data-testid="gs1-exp-date" @change="form.exp_date = $event.detail.value">
...
function submit() {
  const tracking = inferGs1TrackingControls(form)
  ...
  if (tracking.expiry_control && form.expiry_basis === 'MFG' && (!form.shelf_life_days || !form.mfg_date || !form.exp_date)) {
    return uni.showToast({ title: '请补齐保质期和本次日期', icon: 'none' })
  }
  if (tracking.expiry_control && form.expiry_basis === 'INBOUND' && !form.inbound_valid_days) {
...
.empty-tip { display: block; width: 100%; padding: 24rpx; color: #94a3b8; text-align: center; }
.option-grid { display: flex; flex-wrap: wrap; gap: 12rpx; margin-top: 10rpx; }.option-chip { padding: 13rpx 18rpx; border: 1px solid #cbd5e1; border-radius: 999rpx; color: #475569; font-size: 23rpx; }.option-chip.selected { border-color: #2563eb; color: #1d4ed8; background: #eff6ff; font-weight: 700; }
.tracking-block { margin-top: 24rpx; padding-top: 22rpx; border-top: 1px solid #f1f5f9; }.tracking-title, .tracking-help { display: block; }.tracking-title { color: #1e293b; font-size: 26rpx; font-weight: 700; }.tracking-help { margin-top: 4rpx; color: #64748b; font-size: 21rpx; }
