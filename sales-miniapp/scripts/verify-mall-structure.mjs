...
assertIncludes(cartStore, "sale_mini_cart_selection", 'cart selection must persist locally')
assertIncludes(cartStore, 'get selectedItems()', 'cart must expose selected items')
assertIncludes(cartStore, 'get selectedTotalAmount()', 'cart must calculate selected amount')
assertIncludes(cartStore, 'const lines = this.selectedItems', 'cart checkout payload must contain selected items only')
assertIncludes(cartStore, '? previousSelection[item.key] === true', 'cart refresh must preserve existing selections')
assertIncludes(cartStore, ': true', 'new cart items must be selected by default')
assertIncludes(cartStore, 'if (addedItem) this.setSelected(addedItem, true)', 'adding an existing item must select it again')
assertIncludes(cartStore, 'toggleSelection(item)', 'cart item selection must be a toggle')
assertIncludes(cartStore, 'selectAll()', 'cart must support selecting every item')
assertIncludes(cartStore, 'selectNone()', 'cart must support deselecting every item')
...
assertIncludes(indexPage, 'v-model="searchKeyword"', 'home functional search input')
assertIncludes(indexPage, '@confirm="doSearch"', 'home keyboard search action')
...
assertIncludes(indexPage, 'v-for="item in home.categories"', 'home shows every returned major category')
assertIncludes(indexPage, '<swiper', 'home banner follows categories')
assertIncludes(indexPage, "{ key: 'hot', label: '热卖' }", 'home hot feed')
assertIncludes(indexPage, "{ key: 'new', label: '新品' }", 'home new feed')
assertIncludes(indexPage, "{ key: 'recommended', label: '推荐' }", 'home recommended feed')
assertIncludes(indexPage, 'tag: requestedTag', 'home requests strict product tag')
assertIncludes(indexPage, 'onReachBottom', 'home infinite product loading')
assertIncludes(indexPage, 'loadProducts(false)', 'home loads the next product page')
assertIncludes(indexPage, 'variant="grid"', 'home two-column product cards')
...
assertNotIncludes(indexPage, 'class="topbar"', 'home search must be the first content section')
assertNotIncludes(indexPage, 'class="fulfillment"', 'home must not place fulfillment above categories')
assertNotIncludes(indexPage, 'class="channel-grid"', 'home uses fixed major categories instead of channels')
assertNotIncludes(indexPage, '<CartBar', 'home must not show floating checkout bar')
assertNotIncludes(indexPage, '去结算', 'home must not show checkout action')
...
assertNotIncludes(loginPage, '服务地址', 'login must not expose server address')
assertNotIncludes(loginPage, 'serverUrl', 'login must use configured server address')
...
assertIncludes(categoryPage, "const ALL_MIDDLE_ID = 'all-middle'", 'category middle level')
assertIncludes(categoryPage, "const ALL_SMALL_ID = 'all-small'", 'category small level')
...
assertIncludes(categoryPage, 'majorCategories', 'category top major navigation')
assertIncludes(categoryPage, 'enable-flex', 'category major navigation horizontal scrolling')
assertIncludes(categoryPage, '@wheel="handleMajorWheel"', 'category major navigation desktop wheel scrolling')
assertIncludes(categoryPage, '@mousedown="startMajorDrag"', 'category major navigation desktop drag scrolling')
assertIncludes(categoryPage, '@click="openMajorPanel"', 'category all-major panel trigger')
assertIncludes(categoryPage, 'v-if="majorPanelOpen"', 'category all-major panel visibility')
assertIncludes(categoryPage, '全部大类', 'category all-major panel title')
assertIncludes(categoryPage, '收起', 'category all-major panel collapse action')
assertIncludes(categoryPage, 'selectMajorFromPanel', 'category all-major panel selection')
assertNotIncludes(categoryPage, '@click="selectMajor(ALL_CATEGORY_ID)"', 'all-major button must not select all products')
assertIncludes(categoryPage, 'middleCategories', 'category left middle navigation')
assertIncludes(categoryPage, 'smallCategories', 'category small chips')
assertIncludes(categoryPage, 'item.image_url', 'category image with placeholder')
assertIncludes(categoryPage, 'activeCategoryId', 'category subtree product filter')
assertIncludes(categoryPage, 'ordering: ordering.value', 'category product ordering')
assertIncludes(categoryPage, 'only_stock: onlyStock.value', 'category stock filter')
assertIncludes(categoryPage, 'config_id=${product.config_id}', 'category product detail context')
assertNotIncludes(categoryPage, '<CartBar', 'category must not show floating checkout bar')
assertNotIncludes(categoryPage, '去结算', 'category must not show checkout action')
...
assertIncludes(productDetailPage, 'v-if="serviceText"', 'empty product service section must stay hidden')
assertIncludes(productDetailPage, 'product.order_uom_name', 'product detail must display the unit name')
assertIncludes(productDetailPage, '商品合计', 'product detail quantity total')
assertIncludes(productDetailPage, 'lineTotal', 'product detail reactive quantity total')
assertIncludes(productDetailPage, 'favorite-action', 'product detail bottom favorite action')
assertIncludes(productDetailPage, "isFavorite ? '♥' : '♡'", 'product detail favorite heart icon')
assertIncludes(productDetailPage, '起购', 'product detail retail purchase rule copy')
assertIncludes(productDetailPage, 'v-if="rules.enabled"', 'product detail purchase rule visibility switch')
assertIncludes(productDetailPage, ':min="purchaseMin"', 'product detail effective minimum quantity')
assertIncludes(productDetailPage, ':step="purchaseStep"', 'product detail effective quantity increment')
...
assertIncludes(cartPage, 'item.order_uom_name', 'cart must display the unit name')
assertIncludes(cartPage, ':min="quantityMin(item)"', 'cart effective minimum quantity')
assertIncludes(cartPage, ':step="quantityStep(item)"', 'cart effective quantity increment')
...
assertIncludes(cartPage, 'cart.selectedTotalAmount', 'cart summary must use selected amount')
assertIncludes(cartPage, ':disabled="cart.noneSelected"', 'cart checkout must be disabled without selected items')
...
assertIncludes(confirmPage, 'v-for="item in cart.selectedItems"', 'order confirm must show selected items only')
assertIncludes(confirmPage, 'cart.selectedTotalAmount', 'order confirm fallback amount must use selected items')
assertIncludes(confirmPage, 'if (!cart.selectedItems.length)', 'order confirm must reject empty selection')
assertIncludes(confirmPage, 'const loadParams = cartId.value ? { cart_id: cartId.value } : {}', 'order confirm cart-id scoped load')
...
assertIncludes(resultPage, 'paymentService.payAndConfirm', 'order result confirmed payment retry')
...
const paymentService = readText('services/payment.js')
assertIncludes(paymentService, 'querySaleMiniWechat', 'payment service active query')
assertIncludes(paymentService, '[1000, 2000, 3000, 5000]', 'payment confirmation polling')
assertIncludes(paymentService, 'confirmed: false', 'payment service unresolved state')
assertIncludes(paymentService, 'paymentConfirmation', 'cancelled client payment query')
assertIncludes(confirmPage, 'paymentService.payAndConfirm', 'checkout confirmed payment')
assertIncludes(orderDetailPage, 'paymentService.payAndConfirm', 'order detail confirmed payment')
assertIncludes(resultPage, '支付结果确认中', 'order result unresolved payment state')
assertNotIncludes(orderDetailPage, 'paymentService.requestPayment', 'detail must not trust client-only payment success')
assertNotIncludes(resultPage, 'paymentService.requestPayment', 'result must not trust client-only payment success')
