    ...
    >
      ...
      <view class="row mt-2">
        <template v-if="tab==='PENDING'">
          <!-- ✅ stop：避免点按钮也触发进入详情 -->
  <!--        <button class="btn btn-sm" @click.stop="approve(o)">审核通过</button>
          <button class="btn btn-sm" @click.stop="cancel(o)">取消</button> -->
		    <button class="btn btn-sm" @click.stop="approve(o)">审核通过</button>
		    <button class="btn btn-sm" @click.stop="reject(o)">退回修改</button>
		    <button class="btn btn-sm" @click.stop="cancel(o)">取消订单</button>
        </template>
...
const canReview = computed(() => (tab.value === TAB.PENDING))
const TAB = {
  PENDING: 'PENDING',
  APPROVED: 'APPROVED',
  REJECTED: 'REJECTED',
...
//   const statusMap = { PENDING:'OWNER_PENDING', APPROVED:'OWNER_APPROVED', REJECTED:'OWNER_REJECTED',CANCELLED: 'CANCELLED', }
...
function queryParams(pageNo=1){
  ...
  const key = String(tab.value || '').trim().toUpperCase()
  const status = statusMap[key]

  console.log('tab=', tab.value, 'approval_status=', status)
...
async function fetch(pageNo=1){
  ...
  loading.value = true
  try{
    // api.orders(params) 你项目里已有（后定义覆盖前定义）
...
// ✅ 从 approvedetail 返回时会触发 onShow，这里刷新列表，状态会更新（原来的待审核单会消失）
onShow(() => {
  if (firstShow.value) {
...
//   const ok = await confirm('确认取消订单？'); if(!ok) return
//   try{
//     await api.cancelOrder(o.id)
//     uni.showToast({ title:'已取消', icon:'none' })
//     list.value.results=(list.value.results||[]).filter(x=>x.id!==o.id)
//   }catch(e){
//     uni.showToast({ title:e?.data?.detail||'取消失败', icon:'none' })
//   }
// }
...
//   try{
//     await api.ownerReject(o.id)
//     uni.showToast({ title:'已退回修改', icon:'none' })
//     list.value.results = (list.value.results || []).filter(x => x.id !== o.id)
//   }catch(e){
//     uni.showToast({ title:e?.data?.detail || '退回失败', icon:'none' })
//   }
// }

// =================== 操作函数 ===================
async function approve(o){
  const ok = await uni.showModal({ title:'确认', content:'确认审核通过？' })
  ...
  try{
    ...
    uni.showToast({ title: e?.data?.detail || e?.message || '操作失败', icon:'none' })
  }finally{
...
async function reject(o){
  const ok = await uni.showModal({ title:'确认', content:'确认退回业务员修改？' })
  if(!ok.confirm) return
  reviewing.value = true
  try{
    await api.ownerReject(o.id)
...
async function reload(pageNo=1){
  try{
    const res = await api.orders(queryParams(pageNo))
    list.value = res
  }catch(e){
    uni.showToast({ title: e?.data?.detail || e?.message || '加载失败', icon:'none' })
  }
}

async function cancel(o){
  const ok = await uni.showModal({ title:'确认', content:'确认取消订单？' })
  if(!ok.confirm) return
  reviewing.value = true
  try{
    await api.cancelOrder(o.id)
