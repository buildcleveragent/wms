		...
		methods: {
			...
			onDevice(){
				...
				uni.onBluetoothDeviceFound(function(devices) {
							...
							console.log(re.devices[0].name + "  " + re.devices[0].deviceId)
								...
								if(re.devices[0].deviceId == "DC:0D:30:70:23:75"){
									console.log("==============================================")
									console.log("====================  SPP ====================")
									console.log("==============================================")
								}
								...
								if (name != "未知设备" && name != "" && name != "null") {
									let nameUI = re.devices[0].name+"(" +re.devices[0].deviceId+")"
			...
			senBlData(deviceId, serviceId, characteristicId,uint8Array) {
				...
				function realWriteData(sendloop, i) {
					...
					console.log('writeBLE() deviceId = [' + deviceId + ']  serviceId = [' + serviceId + '] characteristics=[' +characteristicId+ "]")
			...
			senBleLabel() {
				...
				// console.log(command)
			    command.setSize(50, 40)
                // command.setGap(2)//标签模式
				command.setGap(0)//非标签模式
                command.setCls()
				command.setBold(1)//0，不加粗  1，加粗
                command.setText(50, 10, "TSS24.BF2", 1, 1, "打印测试（加粗）")
				command.setBold(0)
				command.setText(260, 10, "TSS24.BF2", 1, 1, "打印测试")
				command.setText(260, 50, "TSS24.BF2", 2, 2, "打印测试")
                command.setQR(50, 50, "L", 5, "A", "977767937@qq.com")
				command.setBar(50, 170, "128", 60, 1,2,2, "1234567890")
