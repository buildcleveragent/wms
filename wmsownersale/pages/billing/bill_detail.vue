    ...
    <template v-if="bill">
      <view class="filter-card">
        ...
        <view class="filter-actions">
          ...
          <button class="btn-ghost" :disabled="exporting" @click="exportBill">
            {{ exporting ? '导出中...' : '导出账单' }}
          </button>
...
async function exportBill() {
  if (!billId.value || exporting.value) return
  exporting.value = true
  try {
    await api.billingBillExport(billId.value)
  } catch (error) {
    console.error('export billing bill detail failed:', error)
  } finally {
...
function goBackOverview() {
  ...
  if (previousRoute === 'pages/billing/overview') {
    uni.navigateBack()
    return
  }
