  ...
  <view class="page">
    ...
    <view class="filter-card">
      ...
      <view class="toolbar-actions">
        ...
        <button class="list-btn" @click="openBillList">账单列表</button>
      </view>
    </view>
    ...
    <template v-else-if="selectedPeriod">
      ...
      <view class="section">
        ...
        <view class="trend-summary-grid">
          <view class="trend-summary-card">
            <view class="trend-summary-label">峰值日</view>
            <view class="trend-summary-value">{{ trendSummary.peakDate || '-' }}</view>
            <view class="trend-summary-sub">{{ money(trendSummary.peakTotal) }}</view>
          </view>
          <view class="trend-summary-card">
            <view class="trend-summary-label">日均费用</view>
            <view class="trend-summary-value">{{ money(trendSummary.avgTotal) }}</view>
            <view class="trend-summary-sub">按当前趋势行口径</view>
          </view>
          <view class="trend-summary-card">
            <view class="trend-summary-label">活跃天数</view>
            <view class="trend-summary-value">{{ trendSummary.activeDays }}</view>
            <view class="trend-summary-sub">有费用发生的日期</view>
          </view>
        </view>
      ...
      <view class="bill-card">
        ...
        <view class="bill-actions">
          ...
          <button class="bill-btn ghost" :disabled="!currentBill || exportingBill" @click="exportCurrentBill">
            {{ exportingBill ? '导出中...' : '导出账单' }}
          </button>
        </view>
      </view>
...
const trendSummary = computed(() => {
  ...
  const peak = trendRows.value.reduce((best, item) => (item.total > best.total ? item : best), trendRows.value[0])
  const total = trendRows.value.reduce((sum, item) => sum + item.total, 0)
  return {
    peakDate: peak.serviceDate,
    peakTotal: peak.total,
    avgTotal: total / trendRows.value.length,
    activeDays: trendRows.value.filter((item) => item.total > 0).length,
  }
})
...
function openBillList() {
  ...
  uni.navigateTo({
    url: query ? `/pages/billing/bills?${query}` : '/pages/billing/bills',
  })
}
...
async function exportCurrentBill() {
  if (!currentBill.value?.id || exportingBill.value) return
  exportingBill.value = true
  try {
    await api.billingBillExport(currentBill.value.id)
  } catch (error) {
    console.error('export current bill failed:', error)
  } finally {
    exportingBill.value = false
  }
}
