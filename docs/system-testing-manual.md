...
| `allapp/products/test_sku_sequence.py` | 5 | 货主范围内仓库SKU编码序列递增；软删除不释放序号；迁移从有效和软删除商品初始化；更新不消耗序号；失败插入不消耗序号。 |
...
| `allapp/reports/test_etl_operations.py` | 9 | 全量 ETL 幂等和业务里程碑；计费事实金额更新与普通作废同步；保留软删除商品的已过账流水；排除内部上架移动和取消发运；同一仓库SKU编码保持源订单行；日期窗口重建；全量失败回滚；增量更新；失败不推进 watermark。该文件标记为 `integration`。 |
...
| `allapp/pos/test_checkout_concurrency.py` | 5 | 同一仓库SKU编码并发结账不超卖；反向商品顺序不死锁；POS 与普通拣货串行；作废与结账串行；退货与结账串行。该文件依赖真实事务和 MySQL 行锁。 |
...
测试配置的优先级为“命令行或 CI 显式环境变量 > `.env.test.local` > `.env`”，
因此可以用显式 `DB_TEST_NAME` 选择独立测试库，而不会被本地文件覆盖。
...
不要对半迁移库执行 `migrate --fake`，也不要为了迁就测试残留而修改历史迁移。
MySQL 的 DDL 会自动提交，中断可能留下“字段已创建但迁移记录未写入”的状态；
重建专用测试卷是可重复且可核验的恢复方式。

首次完整建库在当前项目上可能需要 11 分钟以上，期间 pytest 可能长时间没有
输出。只要 MySQL 容器仍健康且进程仍在运行，应继续等待迁移完成，不要因数分钟
无输出就中止。完成首次建库后，后续 `--reuse-db` 运行会明显更快。
