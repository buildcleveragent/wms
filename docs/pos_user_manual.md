...
- 如需指定客户，客户资料需提前维护，并且客户属于当前用户绑定的货主。
- 如果结账时未指定客户，系统会自动使用当前货主下 `code=CASH` 的散客客户；不存在时自动创建 `CASH / 散客`。
...
- `customer_id`：可选，客户 ID；不传时使用当前货主下的散客客户 `CASH`。
...
如果未传 `customer_id`，系统会自动使用当前货主下 `code=CASH` 的散客客户；如果该客户不存在，会自动创建 `CASH / 散客`。
...
- `items` 不能为空。
- `customer_id` 如果传入，对应客户必须存在，并且属于当前货主；如果不传，系统使用散客客户。
...
处理：确认客户资料存在，并且客户 `owner` 与当前用户 `owner` 一致；普通散客收银可以不传 `customer_id`。
