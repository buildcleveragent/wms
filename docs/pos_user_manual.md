...
- 商品资料已维护，状态为启用。
- 货主商品编码、仓库SKU编码、标准贸易条码、单品条码、箱条码或包装条码已按实际扫码需要维护。标准贸易条码对应 `gtin` 字段，支持 GTIN/EAN/UPC。
- 商品价格字段已维护：
  - `price`：建议售价。
  - `min_price`：最低成交价，可为空。
...
条码会通过统一商品标识注册表精确匹配以下当前有效来源：

- 货主商品编码和仓库SKU编码。
- 标准贸易条码、零码、箱码、其他条码及其仍有效的历史记录。
- 包装层级条码及其仍有效的历史记录。
- 各来源系统当前有效的外部系统商品编码。

条码、商品及关联包装都必须启用、未软删除且处于有效期。停用、过期和尚未生效的标识不会作为可售商品命中；历史记录仍永久保留和占用编码。箱码和包装层级条码会返回其创建时固化的包装单位与基础单位换算数量，新旧箱规因此可以同时扫码且各自保持原换算值。
...
`search` 支持按商品名称、规格、货主商品编码、仓库SKU编码、全部当前有效条码历史和全部来源系统的当前有效外部标识进行不区分大小写的模糊查询。退役、过期、尚未生效或关联无效包装的标识不会命中。
